...
public class LoginController {
	...
	public Result<JSONObject> login(@RequestBody SysLoginModel sysLoginModel){
		...
		//1. 校验用户是否有效
		LambdaQueryWrapper<SupplierAccount> queryWrapper = new LambdaQueryWrapper<>();
		queryWrapper.eq(SupplierAccount::getUsername, username);
		SupplierAccount sysUser = supplierAccountService.getOne(queryWrapper);
		result = supplierAccountService.checkUserIsEffective(sysUser);
		if(!result.isSuccess()) {
			return result;
		}
	 ...
	 * @return
	 */
	private Result<JSONObject> supplierInfo(SupplierAccount sysUser, Result<JSONObject> result) {
		String syspassword = sysUser.getPassword();
		String username = sysUser.getUsername();
		JSONObject obj = new JSONObject();
		// 生成token
