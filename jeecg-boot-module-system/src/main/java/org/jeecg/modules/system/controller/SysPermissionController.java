...
//	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/getSupplierPermissionByToken", method = RequestMethod.GET)
	public Result<?> getSupplierPermissionByToken(HttpServletRequest request) {
		Result<JSONObject> result = new Result<JSONObject>();
		try {
			//直接获取当前用户不适用前端token
			LoginUser loginUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
			...
			LambdaQueryWrapper<SupplierPermission> queryAll = new LambdaQueryWrapper<SupplierPermission>();
			queryAll.eq(SupplierPermission::getDelFlag, CommonConstant.DEL_FLAG_0);
			queryAll.orderByAsc(SupplierPermission::getSortNo);
			List<SupplierPermission> permissionList = supplierPermissionService.list(queryAll);
			List<SysPermission> metaList = new ArrayList<>();
			for(SupplierPermission item : permissionList) {
				SysPermission entity = new SysPermission();
				BeanUtils.copyProperties(item, entity);
				metaList.add(entity);
			}
			if(!PermissionDataUtil.hasIndexPage(metaList)){
				SupplierPermission indexMenu = supplierPermissionService.list(new LambdaQueryWrapper<SupplierPermission>().eq(SupplierPermission::getName,"首页")).get(0);
				SysPermission entity = new SysPermission();
				BeanUtils.copyProperties(indexMenu, entity);
				metaList.add(0,entity);
			}
			JSONObject json = new JSONObject();
			JSONArray menujsonArray = new JSONArray();
			this.getPermissionJsonArray(menujsonArray, metaList, null);
			...
			query.eq(SupplierPermission::getMenuType, CommonConstant.MENU_TYPE_2);
			//query.eq(SysPermission::getStatus, "1");
			List<SupplierPermission> allAuthListSupp = supplierPermissionService.list(query);
			List<SysPermission> allAuthList = new ArrayList<>();
			for(SupplierPermission item : allAuthListSupp) {
				SysPermission entity = new SysPermission();
				BeanUtils.copyProperties(item, entity);
				allAuthList.add(entity);
			}
			JSONArray allauthjsonArray = new JSONArray();
			this.getAllAuthJsonArray(allauthjsonArray, allAuthList);
