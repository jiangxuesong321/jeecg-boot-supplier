...
//		SysUser user = this.sysUserService.getOne(new LambdaQueryWrapper<SysUser>().eq(SysUser::getUsername, username));
        SupplierAccount user = iSupplierAccountService.getOne(new LambdaQueryWrapper<SupplierAccount>().eq(SupplierAccount::getUsername, username));
		if(user==null) {
			return Result.error("用户不存在！");
		}
        //update-begin---author:wangshuai ---date:20220316  for：[VUEN-234]修改密码添加敏感日志------------
...
//	}
    ...
    public Result<?> updateAvatar(@RequestBody SupplierAccount json) {
        SupplierAccount account = iSupplierAccountService.getById(json.getId());
        account.setAvatar(json.getAvatar());
        return  Result.ok(iSupplierAccountService.updateById(account));
    }
